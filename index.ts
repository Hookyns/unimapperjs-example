...
setImmediate(async function () {
	...
	let teachers = await Teacher.getAll<Teacher>()
	...
	let startsWith = "P";
	let students = await Student.getAll<Student>()
	...
	let onlyActive = true;
	let activeSubjectsCount = await Subject.getAll<Subject>()
		.filterIf(s => s.active === true, onlyActive) // if (onlyActive) { .filter(s => s.active === true) }
		.count()
		.exec();

	let subjectNames = await Subject.getAll<Subject>()
		.sort(s => s.name)
		.map(s => s.name) // select only names ; in SQL SELECT name FROM Subject
		.exec();

	let subjectMap = await Subject.getAll<Subject>()
		.sort(s => s.name)
		.map(s => ({
			id: s.id,
			name: s.name
